...
def main():
    ...
    #with open('data_v3.csv', 'r',newline='') as csvfile:
        #spamreader = csv.DictReader(csvfile)
        #print(len(csv))
    database = client.recommender
    post = database.data_lagu
    points = [
        Point([post['popularity track'], post['energy'],
               post['valence'], int(post['release date']),
               post['index']])
        for post in post.find()
            #makePoint(dimensions, float(row['popularity track']), float(row['popularity artist'])) 
   ]
    ...
    for i, c in enumerate(clusters):
        for p in c.points:
            ...
            listc = []
            listc = rows.split(', ')
            #print (" Cluster: ", i, "\t Point :", listc[0], " ", listc[3][1:1])
            json = {
                "cluster" : i,
                "average" : float(((float(listc[0])/100) + float(listc[1]) + float(listc[2])))
                }
            #print (listc[4][10:-2])
            collection.update({'index': int(listc[4])},
                                   {"$set":json})
            average = average + float(((float(listc[0])/100) + float(listc[1]) + float(listc[2])))
        clust = {
            "cluster" : i,
            "average" : average
            }
        cluster.update({'cluster':int(i)},clust, upsert=True)
        average = 0           
            
                #writers.writerow([i,listc[0],listc[1], listc[2],listc[3],listc[4][1:-1], listc[5][1:-1]])
...
class Cluster(object):
    ...
    def calculateCentroid(self):
        ...
        coords = [[p.coords[0],p.coords[1]] for p in self.points]
        #print('test')
...
def plotClusters(data, dimensions):
    ...
    for i, c in enumerate(data):
        ...
        if dimensions == 2:
            ...
            trace['x'], trace['y'], namatrack, namaartist = zip(*cluster_data)
            trace['mode'] = 'markers'
            trace['marker'] = {}
            trace['marker']['symbol'] = i
            trace['marker']['size'] = 12
            trace['name'] = namacluster[i] #"Cluster2 " + str(i)
            ...
            centroid['name'] = "pusat centroid : " + namacluster[i] #"Cluster2 " + str(i)
